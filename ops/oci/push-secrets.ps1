...
try {
    ...
    ssh -i $SshKeyPath "${SshUser}@${HostName}" "sed -i 's/\r$//' /tmp/convy-api.env && sudo install -m 600 -o root -g root /tmp/convy-api.env /opt/convy/shared/api.env && sudo install -m 644 -o root -g root /tmp/convy-firebase-admin.json /opt/convy/shared/firebase-admin.json && rm -f /tmp/convy-api.env /tmp/convy-firebase-admin.json"
}
finally {
