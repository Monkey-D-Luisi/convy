param(
    ...
    [string] $SshKeyPath = "$env:USERPROFILE\.ssh\convy_oci_deploy",
...
if ([string]::IsNullOrWhiteSpace($PostgresPassword)) {
    $existingPostgresPasswordCommand = "if sudo test -f /opt/convy/shared/api.env; then sudo sed -n 's/^POSTGRES_PASSWORD=//p' /opt/convy/shared/api.env | head -n 1; fi"
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("convy-oci-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    ssh -i $SshKeyPath "${SshUser}@${HostName}" "sed -i 's/\r$//' /tmp/convy-api.env && sudo install -m 600 -o root -g root /tmp/convy-api.env /opt/convy/shared/api.env && sudo install -m 600 -o root -g root /tmp/convy-firebase-admin.json /opt/convy/shared/firebase-admin.json && sudo chmod 600 /opt/convy/shared/firebase-admin.json && rm -f /tmp/convy-api.env /tmp/convy-firebase-admin.json"
}
finally {
