...
public class UpdateItemCommandHandlerTests
{
    ...
    public UpdateItemCommandHandlerTests()
    {
        ...
        _handler = new UpdateItemCommandHandler(_itemRepository, _listRepository, _householdRepository, _userRepository, _currentUser, _notifications, _activityLogger);
    }
    ...
    public async Task Handle_WithValidData_ReturnsSuccess()
    {
        ...
        var item = new ListItem("Milk", list.Id, _userId, 2, "liters", "Old note");
        ...
        await _notifications.Received(1).NotifyItemUpdated(household.Id, Arg.Any<ListItemDto>(), Arg.Any<CancellationToken>());
    }
