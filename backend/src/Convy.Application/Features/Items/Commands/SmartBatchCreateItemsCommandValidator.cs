...
public class SmartBatchCreateItemsCommandValidator : AbstractValidator<SmartBatchCreateItemsCommand>
{
    public SmartBatchCreateItemsCommandValidator()
    {
        RuleFor(x => x.ListId)
        ...
        RuleFor(x => x.Items)
            ...
            .Must(items => items.Count <= 20).WithMessage("Cannot process more than 20 items at once.");

        RuleForEach(x => x.Items).ChildRules(item =>
        {
            item.RuleFor(i => i.Title)
                .NotNull().WithMessage("Item title is required.")
